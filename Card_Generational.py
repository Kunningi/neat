...
class Card_Generational():
    ...
    def __init__(self, population_size, p_mutation, p_inherit, p_reproduce):
        ...
        self.initial_gene_pool = np.random.choice(a=[False,True], size=(self.population_size, self.genotype_length))        #initializes gene pool with booleans for each gene

        self.evolved_gene_pool = np.zeros((self.population_size, self.genotype_length))                                      #updates the gene pool after selection/reproduction
    ...
    def fitness(self, i):
        ...
        m = 1
        card = 1
        for gene in self.initial_gene_pool[i]:
            if gene == True:
                s += card
            elif gene == False:
                m *= card
        ...
        for i in range(self.population_size):
            self.fit[i] = 1 - (s_error + m_error) / 2
